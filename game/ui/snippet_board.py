    ...
    def _wrap_text(self, font, text, max_w):
        ...
        for tokens in tokens_by_line:
            ...
            for token in tokens:
                ...
                while current and font.size(current)[0] > max_w:
                    split_index = len(current)
                    while split_index > 0 and font.size(current[:split_index])[0] > max_w:
                        split_index -= 1
                    if split_index == 0:
                        break
                    wrapped.append(current[:split_index])
...
class SnippetBoard:
    ...
    def __init__(self, rect: pygame.Rect, fonts):
        ...
        self._slot_line_y: int | None = None
        self._last_slot: int | None = None
        self._slot_hysteresis_px = 14  # px to move past a boundary before switching slots
    ...
    def update(self, dt: float):
        ...
        if self._smooth_top is None:
            ...
            self._smooth_top += (target_top - self._smooth_top) * 0.18  # softer easing for drag follow
        ...
        if my < self._list_top + EDGE and self.scroll_y > 0:
            ...
            self.scroll_y = min(self._max_scroll, self.scroll_y + 8)
